...
#usamos bcftools para ver ver filtrar y extraer variantes
bcftools view \
#decimos en que región genómica se encuentra lo que buscamos: posiciones de rs429358 y rs7412 en GRCh37/hg19
-r 19:45411941,19:45412079 \
#enunciamos que sólo acepte sitios donde el camibo es bialélico, solo SNPs
-m2 -M2 -v snps \
#representa el archivo de entrada vcf
$VCF \
#enunicamos que el archivo de salida sea un .vcf.gz archivo comprimido con bgzip
-O z \
#y que el archivo de salida se guarde en la carpeta de analysis, obteniendo un .vcf recortado con solo las variantes de APOE para todos los individuos
...
#usamos bcftools para ver ver filtrar y extraer variantes
bcftools view \
#indicamos la lista de muestras que quedarían (las extrajimos en el paso anterior del loop)
-S analysis/${SUPER}.samples.txt \
#indicamos el .vcf grande
analysis/APOE.vcf.gz \
#enunicamos que el archivo de salida sea un .vcf.gz archivo comprimido con bgzip
-O z \
#y que el archivo de salida se guarde en la carpeta de analysis, obteniendo un .vcf recortado con solo las variantes de APOE para una superpoblación
...
bcftools query \
#como sera el formato de salida? para todas las muestras del vcf repite el genotipo+tabulacion
-f '[%GT\t]\n' \
#de donde? archivo de entrada
...
vcftools \
#indicando el archivo de entrada y que esta comprimido en gzip
--gzvcf analysis/${SUPER}.vcf.gz \
#calcula estadisticas HW dejando un archivo .hwe
--hardy \
#indica el prefilo de la salida
--out analysis/${SUPER}
...
#indicando el archivo de entrada y que esta comprimido en gzip
--gzvcf analysis/${SUPER}.vcf.gz \
#diversidad nucletidica por sitio
--site-pi \
#enunciar el archivo de salida
--out analysis/${SUPER}
...
#indicando el archivo de entrada y que esta comprimido en gzip
--gzvcf analysis/${SUPER}.vcf.gz \
#con het calculamos estadisticas de heterocigosidad por sitio
--het \
#generamos la salida .het
--out analysis/${SUPER}_het \

#luego calculamos el promedio de las heterocigosidad observadas entre los SNPs y se guarda en PI, se lee con awk sin el header y hace que se calcule la heterocigosidad observada ((total sitios menos homocigotos observados) dividido en el total de sitios ) y el numero de SNPs
#luego en el end entonces hacemos que si hay SNPs imprima la suma dividido en el n generando el promedio de las heterocigosidades observaadas
HO=$(awk '
NR>1{
ho = ($4 - $2)/$4
sum += ho
